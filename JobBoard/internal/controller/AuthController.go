...
type RegisterRequest struct {
	...
	IsEmployer bool   `form:"is_employer" validate:"required"`
}
...
func (ac *AuthController) VerifyEmail(c *fiber.Ctx) error {
	...
	JWT, err := utils.GenerateToken(int(updateResult.ID), updateResult.Avatar, updateResult.Email, updateResult.Name, updateResult.Token, updateResult.IsEmployer)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
	...
	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		...
		"user": fiber.Map{
			...
			"IsEmployer": updateResult.IsEmployer,
		},
	})
...
func (ac *AuthController) Login(c *fiber.Ctx) error {
	...
	JWT, err := utils.GenerateToken(int(user.ID), user.Avatar, user.Email, user.Name, user.Token, user.IsEmployer)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error":  "Failed to generate token",      // Error message.
			"status": fiber.StatusInternalServerError, // Internal server error status code.
